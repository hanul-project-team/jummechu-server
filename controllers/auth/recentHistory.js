...
const callOpenAiForKeywords = async (prompt) => {
  try {
    ...
    if (response.data) {
      ...
      if (typeof response.data === "string") {
        try {
          ...
          console.warn(
            "Backend: OpenAI 응답이 유효한 JSON 문자열이 아닙니다. 원시 문자열로 처리 시도.",
            parseError
          );
    ...
    console.error(
      "Backend: OpenAI 키워드 생성 API 호출 실패:",
...
export const addRecentView = async (req, res) => {
  ...
  try {
    ...
    if (finalKeywords.length === 0 || !hasFoodCategory) {
      console.log(
        "키워드가 비어있거나 음식 카테고리가 없어 OpenAI 키워드 생성/보완 시도 중..."
      );
      const openaiPrompt = `
      ...
      if (generated && generated.length > 0) {
        ...
        if (finalKeywords.length === 0) {
          ...
          console.log(
            "OpenAI 키워드 생성 실패, 가게 이름을 키워드로 사용:",
            finalKeywords
          );
        } else {
          console.log(
            "OpenAI 키워드 생성 실패, 기존 키워드 유지:",
            finalKeywords
          );
        }
      }
      ...
      console.log(
        "음식 카테고리 키워드가 이미 존재하여 OpenAI 호출을 건너뜁니다:",
        finalKeywords
      );
    }
